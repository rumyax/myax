...
RUN = conf.get('run', {})
SSH = conf.get('ssh', {})
SSH_KEYS_PATH = conf.get('ssh-keys-path', '')


def usage():
    print('Usage: myax {run|open|ssh|i|back|init|commit|post|amend|push}')
    ...
    print('    ssh <key>  - SSH into the host associated with the key from conf.json')
...
def myax(arg):
    if arg == 'run' or arg == 'open':
        ...
        if not SSH_KEYS_PATH:
            printError('ssh-keys-path not found')
            return
        ssh_key_file = os.path.join(SSH_KEYS_PATH, f'{key}.pem')
        if not os.path.isfile(ssh_key_file):
            printError('ssh key file not found')
            return
        run(f'ssh -i "{ssh_key_file}" -o "StrictHostKeyChecking=no" {target}')
